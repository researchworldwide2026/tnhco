...
```mermaid
graph TB
    subgraph "Gründung 13.10.2023"
        BOARD1["Maya Rohner-Andermatt<br/>Kassierin"]
        BOARD2["Peter Zehnder<br/>Vizepräsident"]
        RI1["René Alfred Immoos<br/>Mitglied"]
        VM1["Valérie Hélène Manuel<br/>Mitglied"]
        JM1["Josef Martin Marty<br/>Mitglied"]
    end

    subgraph "Mutation 19.10.2023"
        RI2["René Immoos<br/>→ Vizepräsident"]
        OUT1["❌ Peter Zehnder<br/>ausgeschieden"]
        OUT2["❌ Maya Rohner<br/>ausgeschieden"]
    end

    subgraph "Mutation 22.12.2023 🚨"
        OUT3["❌ René Immoos<br/>ausgeschieden"]
        OUT4["❌ Josef Marty<br/>ausgeschieden"]
        OUT5["❌ Valérie Manuel<br/>ausgeschieden"]
        NEW1["Jacqueline Wachter<br/>→ Vizepräsidentin"]
        NEW2["Peter... [Name abgeschnitten]"]
    end

    subgraph "2025"
        MOVE["Sitz: Menzingen → Neuheim<br/>Industriestrasse 3"]
    end
    ...
    style OUT3 fill:#ff4444,color:#fff
    style OUT4 fill:#ff4444,color:#fff
    style OUT5 fill:#ff4444,color:#fff
    style NEW1 fill:#a8e6cf,color:#000
```
...
```mermaid
graph TB
    subgraph "OFFIZIELL (Handelsregister)"
        JW["Jacqueline Wachter<br/>Vizepräsidentin"]
        PX["Peter...<br/>[Name nicht vollständig]"]
    end

    subgraph "FRÜHERE (ausgeschieden 12/2023)"
        RI["René Immoos ❌"]
        JM["Josef Marty ❌"]
        VM["Valérie Manuel ❌"]
    end
    ...
    subgraph "IM HINTERGRUND"
        GNB["Georges Nicolas Bolliger<br/>'Entscheidungsbefugnis'<br/>Unbekannte Funktion<br/>⚠️ KEIN VR-Mandat!"]
        AP["Alexander Paramonov<br/>King of Kings<br/>M1 Chief Treasurer"]
    end

    subgraph "VERBUNDENE ENTITÄTEN"
        TNHCO["🏛️ TNHCO<br/>CHE-242.461.997"]
        GOLDENERA["☀️ GoldenEra<br/>goldenera.global"]
        SPD_ROD[" SPD ROD<br/>Slowakei"]
        WSBOH["🏦 WSBOH<br/>wsboh.org"]
    end

    JW -->|"Verwaltet"| TNHCO
    GNB -.->|"Kontrolliert?"| TNHCO
    GNB -.->|"Entscheidungsbefugnis"| TNHCO
    AP -->|"Chief Treasurer<br/>Program Sunrise"| TNHCO
    TNHCO -->|"Strategic Partner"| GOLDENERA
    TNHCO -->|"Strategic Partner"| SPD_ROD
    GOLDENERA -->|"Quick Link"| WSBOH
    SPD_ROD -->|"Zlatá éra Kult"| AP

    style GNB fill:#ffd700,color:#000,stroke:#cc9900,stroke-width:3px
    style AP fill:#ff4444,color:#fff
    style JW fill:#a8e6cf,color:#000
    style RI fill:#999,color:#fff
    style JM fill:#999,color:#fff
    style VM fill:#999,color:#fff
```
