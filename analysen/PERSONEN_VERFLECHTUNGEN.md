...
graph TB
    subgraph "🇷🇺 RUSSLAND — Ideologischer Kern"
        ...
        TARASKIN["👤 Sergey Taraskin<br/>SSSR-Gründer<br/>»Präsident der UdSSR«"]
    end
    ...
    subgraph "🇿🇦 SÜDAFRIKA — Prominentes Opfer?"
        SEXWALE["👤 Tokyo Sexwale<br/>ANC-Veteran<br/>Anzeigeerstatter"]
    end

    subgraph "🇨🇿 TSCHECHIEN — Sekten-Aufklärung"
        SEKTY_TV["📺 sekty.tv<br/>Investigativer Journalismus"]
    end

    PARAMONOV -->|"Dokumenten-Hosting"| TNHCO
    PARAMONOV -->|"Ideologische Führung"| NOD_SSSR
    PARAMONOV -->|"Gründungsmythos"| ZLATA_ERA
    BOLLIGER -->|"FINMA-Verbindung"| TNHCO
    MARTY -->|"FINMA-Verbindung"| TNHCO
    IMMOOS -->|"FINMA-Verbindung"| TNHCO
    MANUEL -->|"FINMA-Verbindung"| TNHCO
    SEXWALE -->|"Opfer/Nutzer der<br/>M1-Erzählung"| PARAMONOV

    TNHCO["🏛️ TNHCO<br/>Terra Nova Helvetica<br/>Genossenschaft<br/>Menzingen ZG"]
    NOD_SSSR["🚩 NOD SSSR<br/>Volksbewegung zur<br/>Befreiung der UdSSR"]
    ZLATA_ERA["✨ Zlatá éra<br/>Goldene Ära<br/>SPD ROD Kult"]

    style PARAMONOV fill:#ff4444,color:#fff,stroke:#cc0000,stroke-width:3px
    style TNHCO fill:#e17055,color:#fff,stroke:#cc0000,stroke-width:2px
    style BOLLIGER fill:#dfe6e9,color:#000
    style MARTY fill:#dfe6e9,color:#000
    style IMMOOS fill:#dfe6e9,color:#000
    style MANUEL fill:#dfe6e9,color:#000
    style SEXWALE fill:#ffd700,color:#000
```
...
graph LR
    ...
    subgraph "Finanz-Konstrukte"
        M1["🏦 International Treasury<br/>Monetary One (M1)"]
        WSB["📄 White Spiritual Boy<br/>R.S.B. Global Corp Inc"]
        WSBT["📄 World Sovereign Bank<br/>Trust"]
        ITMO["📄 International Treasury<br/>Monetary One<br/>(Genossenschaft)"]
    end

    subgraph "Politische Bewegungen"
        NOD["🚩 NOD SSSR<br/>Volksbewegung zur<br/>Befreiung der UdSSR"]
        DOM["🏠 Domov SSSR<br/>UdSSR-Heim"]
        SSSR["⚒️ SSSR / Union der<br/>Slawischen Kräfte"]
    end

    subgraph "Spirituell/Kultisch"
        ZLATA["✨ Zlatá éra<br/>(Goldene Ära)<br/>SPD ROD"]
        JOHANNITER["⚜️ Orden der Johanniter<br/>(selbsternannt)"]
    end

    PARAMONOV -->|"Chef-Schatzmeister"| M1
    PARAMONOV -->|"Präsident/COO"| WSB
    PARAMONOV -->|"Führer"| NOD
    PARAMONOV -->|"Gründer/Prophet"| ZLATA
    PARAMONOV -->|"Selbsternannter<br/>Souverän"| JOHANNITER
    NOD -->|"Umbenennung/Vorgänger"| DOM
    DOM -->|"Verbindung zu"| SSSR

    style PARAMONOV fill:#ff4444,color:#fff,stroke:#cc0000,stroke-width:3px
    style M1 fill:#e17055,color:#fff
    style WSB fill:#e17055,color:#fff
    style ZLATA fill:#6c5ce7,color:#fff
    style NOD fill:#d63031,color:#fff
```
...
graph TB
    ...
    subgraph "MYTHOLOGIE"
        WSB_MYTH["📜 White Spiritual Boy<br/>Mythos vom »Erben des<br/>globalen Finanzsystems«"]
        GOLD["💰 Goldener Sowjetrubel<br/>Gold-ECU<br/>Golddollar"]
        ORDER["⚜️ Orden der Johanniter<br/>als »souveräne<br/>Zentralbank«"]
    end

    subgraph "SCHWEIZ — LEGALES VEHIKEL"
        TNHCO_ENT["🏛️ Terra Nova Helvetica<br/>Genossenschaft<br/>Menzingen ZG"]
        GB["👤 Georges Bolliger"]
        JM["👤 Josef Marty"]
        RI["👤 René Immoos"]
        VM["👤 Valeria Manuel"]
    ...
    subgraph "RUSSLAND — POLITISCHE BEWEGUNG"
        NOD_ORG["🚩 NOD SSSR<br/>Nizhnevartovsk"]
        DOM_ORG["🏠 Domov SSSR<br/>2019: FSB-Razzia"]
        SSSR_ORG["⚒️ SSSR / Union<br/>Slawischer Kräfte<br/>2019: Extremistisch"]
        RS["👤 Rustam Sharipov<br/>NOD-Leiter lokal"]
        ST["👤 Sergey Taraskin<br/>»Präsident der UdSSR«"]
    end

    subgraph "OSTEUROPA — AUSBREITUNG"
        ZL["✨ Zlatá éra<br/>Tschechien/Slowakei"]
        HU["🇭🇺 Ungarische<br/>Dokumente<br/>(HATÁROZAT)"]
        MAKS["📄 maks.hu<br/>Hosting von<br/>M1-Resolutionen"]
    end

    subgraph "OPFER"
        TS["👤 Tokyo Sexwale<br/>Südafrika"]
    end

    AP -->|"Erschafft"| WSB_MYTH
    AP -->|"Propagiert"| GOLD
    AP -->|"Beansprucht"| ORDER
    AP -->|"Führt"| NOD_ORG
    AP -->|"Gründet"| ZL

    NOD_ORG -->|"Vorgänger"| DOM_ORG
    DOM_ORG -->|"Verbindung"| SSSR_ORG
    ST -->|"Gründer"| SSSR_ORG
    RS -->|"Lokaler Leiter"| NOD_ORG

    WSB_MYTH -->|"Dokumente gehostet auf"| TNHCO_ENT
    GOLD -->|"Beworben von"| TNHCO_ENT
    ORDER -->|"Dokumentiert auf"| TNHCO_ENT

    TNHCO_ENT -->|"FINMA-Verbindung"| GB
    TNHCO_ENT -->|"FINMA-Verbindung"| JM
    TNHCO_ENT -->|"FINMA-Verbindung"| RI
    TNHCO_ENT -->|"FINMA-Verbindung"| VM

    HU -->|"Gehostet auf"| TNHCO_ENT
    MAKS -->|"Spiegel-Hosting"| TNHCO_ENT

    TS -->|"Glaubte an"| WSB_MYTH

    style AP fill:#ff4444,color:#fff,stroke:#cc0000,stroke-width:4px
    style TNHCO_ENT fill:#e17055,color:#fff,stroke:#cc0000,stroke-width:2px
    style NOD_ORG fill:#d63031,color:#fff
    style SSSR_ORG fill:#b71c1c,color:#fff
    style TS fill:#ffd700,color:#000
```
...
graph LR
    ...
    subgraph "Schweizer Legalstruktur"
        GB2["Bolliger<br/>🏛️ Verwaltung?"]
        JM2["Marty<br/>🏛️ Verwaltung?"]
        RI2["Immoos<br/>🏛️ Verwaltung?"]
        VM2["Manuel<br/>🏛️ Verwaltung?"]
    end

    subgraph "Russische Basis"
        RS2["Sharipov<br/>🚩 Lokale Organisation"]
        AKTIVISTEN["Aktivisten<br/>👥 Mitgliederwerbung<br/>🚗 Auto-Korsos"]
    end

    subgraph "Dokumentation"
        TNHCO2["tnhco.org<br/>📄 Dokumenten-Host<br/>🌐 Mehrsprachig"]
        MAKS2["maks.hu<br/>📄 Spiegel-Host<br/>🇭🇺 Ungarn"]
    end

    subgraph "Prominente Reichweite"
        TS2["Sexwale<br/>🎯 Multiplikator<br/>🇿🇦 Südafrika"]
    end

    AP2 -->|"Erzählung"| TS2
    AP2 -->|"Dokumente"| TNHCO2
    AP2 -->|"Dokumente"| MAKS2
    AP2 -->|"Führung"| RS2
    RS2 -->|"Mobilisierung"| AKTIVISTEN
    TNHCO2 -->|"betrieben von"| GB2
    TNHCO2 -->|"betrieben von"| JM2
    TNHCO2 -->|"betrieben von"| RI2
    TNHCO2 -->|"betrieben von"| VM2
```
